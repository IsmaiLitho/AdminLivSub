            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="{{ url(Auth::user()->tienda.'/pc/nueva-campania') }}" class="mt-4 mb-4 btn btn-primary"><i class="bi bi-plus-lg"></i> Nueva campaña</a>
                        @if(Session::has('m'))
                        ...
                        @endif
                        <div class="campanias-pc-table" hidden>
                            <table class="table display" id="campanias-pc" style="width:100%">
...
@section('scripts')
    ...
    <script>
        ...
        $('#ambiente').on('change', function() {
            //$('#campanias-pc').find('tbody').remove();
            
            ambiente = $(this).val();
            url = `{{ url('/${tienda}/${ambiente}/pc/getCampanias') }}`;
            //console.log(url);

            $('.campanias-pc-table').removeAttr('hidden',true);

            $('#campanias-pc thead tr').addClass('filters').appendTo('#campanias-pc thead');

            var table = $('#campanias-pc').DataTable({
                ajax: url,
                processing: true,
                serverSide: true,
                //responsive:true,
